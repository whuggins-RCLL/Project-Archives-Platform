...
export default function Sidebar({
  ...
  return (
    <>
      ...
      />
      <aside className={`h-screen w-64 fixed left-0 top-0 z-50 bg-surface-container-low flex flex-col py-8 px-6 space-y-4 transform transition-transform duration-200 lg:z-40 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} ${isDesktopOpen ? 'lg:translate-x-0' : 'lg:-translate-x-full'}`}>
        <button
          onClick={onMobileClose}
        ...
        <div className="mb-8 px-1">
          <div className="flex gap-3 items-center">
            {branding.logoUrl ? (
              <div className="shrink-0 flex h-14 w-14 items-center justify-center rounded-xl border border-outline-variant/25 bg-surface-container-lowest shadow-sm">
                <img
              ...
              <div className="shrink-0 flex h-14 w-14 items-center justify-center rounded-xl bg-primary-container shadow-sm">
                <FolderArchive className="w-7 h-7" style={{ color: 'var(--brand-on-dark)' }} aria-hidden />
              </div>
            )}
            <div className="min-w-0">
              <p className="text-xs text-on-surface-variant uppercase tracking-[0.2em]">
                Workspace
              </p>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 ${
                  isActive
                    ? 'bg-surface-container-lowest text-brand-dark shadow-sm'
                    : 'text-on-surface-variant hover:text-brand-dark hover:bg-surface-container-high'
                }`}
              >
        ...
        <div className="pt-6 border-t border-outline-variant/40 space-y-1">
          {!viewerOnlyMode && (
            <Button
              onClick={() => {
                onNewProject();
          ...
          <button
            ...
            className="w-full flex items-center space-x-3 px-4 py-2 text-on-surface-variant hover:text-error text-xs font-medium"
          >
